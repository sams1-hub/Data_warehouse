...
# Step 1: Connect to Cassandra (Cloud using Scylla)
def connect_to_cassandra_cloud():
    try:
        auth_provider = PlainTextAuthProvider('your_client_id', 'your_client_secret')
        cluster = Cluster(['your-cloud-endpoint'], auth_provider=auth_provider)
        session = cluster.connect()
        print("Connected to Cassandra cloud.")
        return session
    except Exception as e:
        print(f"An error occurred while connecting to Cassandra cloud: {e}")
        return None
...
if __name__ == "__main__":
    ...
    # Step 1: Connect to Cassandra cloud
