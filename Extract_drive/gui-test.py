...
class DriveDownloaderGUI:
    def __init__(self, root):
        ...
        self.files = []
        self.folders_config = load_folder_config()
    ...
    def setup_gui(self):
        ...
        api_key_entry = ttk.Entry(config_frame, textvariable=self.api_key_path, width=40)
        api_key_entry.grid(row=0, column=1, sticky=(tk.W, tk.E), padx=5)
        ttk.Button(config_frame, text="Parcourir", command=self.browse_api_key).grid(row=0, column=2, padx=5)

        # Folder Selection
        ttk.Label(config_frame, text="Sélection Dossier:").grid(row=1, column=0, padx=5)
        self.folder_combo = ttk.Combobox(config_frame, 
                                       textvariable=self.selected_folder,
                                       values=list(self.folders_config.keys()),
                                       width=37)
        self.folder_combo.grid(row=1, column=1, sticky=(tk.W, tk.E), padx=5)
        ttk.Button(config_frame, text="Rafraîchir", command=self.refresh_folders).grid(row=1, column=2, padx=5)

        # Manual Folder ID
        ttk.Label(config_frame, text="OU Folder ID manuel:").grid(row=2, column=0, padx=5)
        folder_id_entry = ttk.Entry(config_frame, textvariable=self.folder_id, width=40)
        folder_id_entry.grid(row=2, column=1, sticky=(tk.W, tk.E), padx=5)
        ttk.Button(config_frame, text="Lister Fichiers", command=self.list_files).grid(row=2, column=2, padx=5)
        ...
        ttk.Button(button_frame, text="Télécharger", command=self.start_download).grid(row=0, column=0, padx=5)
        ttk.Button(button_frame, text="Arrêter", command=self.stop_download).grid(row=0, column=1, padx=5)

        # Configure grid weights
        main_frame.columnconfigure(1, weight=1)

    def on_folder_selected(self, *args):
        """Called when a folder is selected from the dropdown"""
    ...
    def refresh_folders(self):
        """Refresh the folders list from the configuration file"""
        self.folders_config = load_folder_config()
    ...
    def download_process(self, file_id, file_name):
        try:
            ...
            destination_path = os.path.join("data/download", file_name)
